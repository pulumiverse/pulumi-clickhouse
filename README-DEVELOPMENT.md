...
`terraform-provider-clickhouse`.  Substitute
...
* `provider/` contains the Go code used to create the provider as well as
  ...
  * `provider/cmd/pulumi-tfgen-clickhouse`
    generates the Pulumi resource schema (`schema.json`), based on the Terraform
    provider's resources.
  * `provider/cmd/pulumi-resource-clickhouse`
...
1. In `provider/go.mod`, add a reference to the upstream Terraform provider in the `require` section, e.g.
    ...
    ```go
    github.com/foo/terraform-provider-clickhouse v0.4.0
    ```

1. In `provider/resources.go`, ensure the reference in the `import` section uses the correct Go module path, e.g.:

    ```go
    github.com/foo/terraform-provider-clickhouse/clickhouse
    ```
...
1. Create the schema by running the following command:
    ...
    ```text
    warning: resource clickhouse_something not found in provider map; skipping
    warning: resource clickhouse_something_else not found in provider map; skipping
    warning: data source clickhouse_something not found in provider map; skipping
    warning: data source clickhouse_something_else not found in provider map; skipping
    ```
...
1. **Add resource mappings:** For each resource in the provider, add an entry in
   the `Resources` property of the `tfbridge.ProviderInfo`, e.g.:
    ...
    "clickhouse_something":      {Tok: tfbridge.MakeResource(mainPkg, mainMod, "Something")},
    "clickhouse_something_else": {Tok: tfbridge.MakeResource(mainPkg, mainMod, "SomethingElse")},
    ```
...
1. **Add data source mappings:** For each data source in the provider, add an entry in the `DataSources` property of the `tfbridge.ProviderInfo`, e.g.:
    ...
    "clickhouse_something":      {Tok: tfbridge.MakeDataSource(mainPkg, mainMod, "getSomething")},
    "clickhouse_something_else": {Tok: tfbridge.MakeDataSource(mainPkg, mainMod, "getSomethingElse")},
    ```
...
1. **Add provider configuration overrides (not typically needed):** Pulumi's
   ...
   `clickhouse_API_KEY`.  Examples of common
   uses cases:
    ...
    "apikey": {
        Default: &tfbridge.DefaultInfo{
            EnvVars: []string{"clickhouse_API_KEY"},
        },
    ```
...
1. Create a new Pulumi program in the `examples/` directory, e.g.:
    ...
    npm install
    yarn link @pulumiverse/clickhouse
    ```
