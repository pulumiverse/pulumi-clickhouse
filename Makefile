...
SHELL            := /bin/bash
PROJECT          := github.com/pulumiverse/pulumi-clickhouse
...
TFGEN            := pulumi-tfgen-clickhouse
PROVIDER         := pulumi-resource-clickhouse
...
upstream/.git:
	...
	git -c advice.detachedHead=false clone --depth 1 --branch 'v$(PROVIDER_VERSION)' 'https://github.com/ClickHouse/terraform-provider-clickhouse' upstream
